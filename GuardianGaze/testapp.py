...
def extract_features(packet):
    ...
    features.append(packet[IP].proto if IP in packet else 0)  # Protocol
    features.append(packet[IP].len if IP in packet else 0)  # IP packet length
    features.append(packet[TCP].flags if TCP in packet else 0)  # TCP Flags
    features.append(packet[UDP].len if UDP in packet else 0)  # UDP packet length
    return np.array(features).reshape(1, -1)

# Function to predict if a packet is an anomaly
    ...
    def capture_and_analyze(self):
        ...
        while self.is_capturing:
            ...
            for packet in packets:
                ...
                if prediction == 1:
                    messagebox.showwarning("Intrusion Alert", "Anomaly Detected!")
